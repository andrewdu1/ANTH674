...
You can set your baseline factor level (if you don't like the default) using the `relevel()` function.
...
You will see the P-value has gone up compared to that in the t-test.
...
So, lets fit an interaction multiple regression model, where the DV is `mtcars$hp` and the IVs are `mtcars$am` and `mtcars$mpg`.
...
```{r ancova2-solution}
ancova.res <- lm(hp ~ am * mpg, data = mtcars)

plot(mtcars$hp ~ mtcars$mpg, type = "n")
points(mtcars$hp[mtcars$am == 0] ~ mtcars$mpg[mtcars$am == 0], pch = 16, col = "red")
points(mtcars$hp[mtcars$am == 1] ~ mtcars$mpg[mtcars$am == 1], pch = 16, col = "blue")

legend("topright", c("Automatic (0)", "Manual (1)"), pch = c(16, 16), col = c("red", "blue"))

abline(ancova.res$coefficients["(Intercept)"], ancova.res$coefficients["mpg"], col = "red")
abline(ancova.res$coefficients["(Intercept)"] + ancova.res$coefficients["am"], ancova.res$coefficients["mpg"] + ancova.res$coefficients["am:mpg"], col = "blue")
```
...
As before, the intercept is the intercept of the `hp ~ mpg` regression when `am == 0` (the baseline level, which is "Automatic").
The estimated `mpg` coefficient (i.e., the continuous IV) is the corresponding slope for `am == 0`.
...
The interaction term, `am:mpg`, is how much the slope changes as you go from `am == 0` to `am == 1`. 
...
The `am` and `am:mpg` coefficients are significant, telling us that the intercepts and slopes, respectively, are significantly different between `am == 0` and `am == 1`.
And the coefficients tell us by how much the intercepts and slopes are changing (111.88 and -3.56, respectively).
