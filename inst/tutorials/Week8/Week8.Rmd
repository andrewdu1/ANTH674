...
Calling the `glm()` object `glm.binom`, enter `predict(glm.binom, data.frame(wt = seq(1, 6, length.out = 100)))` (the `seq(1, 6, length.out = 100)` simply creates a vector of many values within the range of the IV, so a smooth curve can be plotted).
...
You can then add the fitted logistic curve, using `lines(seq(1, 6, length.out = 100), inv.logit(predict(glm.binom, data.frame(wt = seq(1, 6, length.out = 100)))))`.
