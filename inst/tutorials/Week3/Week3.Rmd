...
There is a lot in this week's tutorial, so feel free to do this one in batches if it's all too much.
...
`ggplot2` works seamlessly with `dplyr`, the wrangling R package I mentioned in last week's tutorial (in fact, `ggplot2` and `dplyr` are part of the larger [`tidyverse` collection of packages](https://www.tidyverse.org)).
So if you learn `dplyr`, it is worth learning `ggplot2`, even though there is a learning curve involved.
There are also some plots that are just easier to create in `ggplot2`.
I myself have only ever used base R, except for a few times when it was easier to generate the complicated plot in `ggplot2`.
...
![](https://www.varsitytutors.com/assets/vt-hotmath-legacy/hotmath_help/topics/normal-distribution-of-data/normal-distribution-1.gif){width="50%"}
...
Categorical data are unordered, qualitative data, where each data point is given a name assigning it to some group.
...
It's a long vector, so you can get a feel for it by using `head(hair_color)` and `table(hair_color)`.
Try this out below.
...
# Click "solution" button for bar plot of hair color
```

```{r hair_bar-solution}
barplot(table(hair_color))
```
...
Ordinal data are *ordered*, qualitative data, where each data point is given a name assigning it to some group.
...
We can use the `levels()` function to see what the factor levels are and what order they're in, i.e., `levels(hair_color)`.
...
# Click solution button for reordering factor levels & creating bar plot
```

```{r hair_order-solution}
hair.color_ordered <- ordered(
  hair_color, 
  levels = c("Blond", "Red", "Brown", "Black")
  )

barplot(table(hair.color_ordered))
```
...
barplot(
  hair.color_sex, 
  beside = TRUE, 
  legend.text = rownames(hair.color_sex)
  )

barplot(
  t(hair.color_sex), 
  beside = TRUE, 
  legend.text = colnames(hair.color_sex)
  )
```
...
Discrete data are defined as those data that are finite or countable values (e.g., integers).
...
FYI, *Paranthropus* is a hominin genus that looked like this:

![](https://humanorigins.si.edu/sites/default/files/styles/grid_thumbnail/public/images/landscape/boisei_JG_Recon_head_CC_3qtr1_lt_l.jpg.webp?itok=Tqa0Dzix){width="50%"}
...
Continuous data are defined as values within a continuous interval (e.g., 2.4575).
...
This takes the form of (1) box plots and violin plots, where each box and violin corresponds to a different category, and (2) density plots, where each density curve corresponds to a different category.
...
To create a box plot, we use the `boxplot()` function, but this function will only accept vectors as arguments in formula format (this is common when creating linear models in R, as we'll see in future lectures/tutorials).
...
vioplot(iris$Petal.Length ~ iris$Species, horizontal = TRUE)
```
...
This gives you more control over how each curve is plotted (e.g., its line type, color, thickness, which curve is plotted on top of others).
...
Sure, we can look at the `Petal.Length` vector and visually pick out the smallest and largest value, but we should automate everything now that we're R coders (i.e., soft code!).
...
Can you figure out how to do this (solution is in the console below)?
...
Make sure to make the lines a different color (e.g., red, blue) or type (e.g., dotted, dashed) to differentiate the curves (using the `col` or `lty` arguments).
...
![](http://www.sthda.com/sthda/RDoc/figure/graphs/linetypes-in-r-line-types.png){width="50%"}
...
```{r dens3-solution}
# using range for xlim
plot(
  density(iris$Petal.Length[iris$Species == "setosa"]), 
  xlim = range(iris$Petal.Length), 
  main = "", 
  xlab = "Petal length (cm)"
  ) 

# using min and max for xlim
plot(
  density(iris$Petal.Length[iris$Species == "setosa"]), 
  xlim = c(
...
# using blue curve for second species
lines(
  density(iris$Petal.Length[iris$Species == "versicolor"]), 
  col = "blue"
  )

# using dashed lines instead
lines(
  density(iris$Petal.Length[iris$Species == "versicolor"]), 
  lty = 2
  ) 

# using red curve for third species
lines(
  density(iris$Petal.Length[iris$Species == "virginica"]), 
  col = "red"
  )

# using dotted lines instead
lines(
  density(iris$Petal.Length[iris$Species == "virginica"]), 
  lty = 3
  ) 

# creating a legend (colored curves)
legend(
  "topright", 
...
# creating a legend (dotted/dashed curves)
legend(
  "topright", 
  legend = unique(iris$Species), 
  lty = c(1, 2, 3)
...
**NB:** `plot()` also accepts arguments using the `~` formula format we encountered before in `boxplot()`. 
So, if I wanted to plot `Petal.Length` as a function of `Petal.Width`, I would enter `plot(iris$Petal.Length ~ iris$Petal.Width)`.
...
Create a legend, so the reader knows what the different colors are.
...
[Click this link for solution code if it's cut off above.](https://raw.githubusercontent.com/andrewdu1/ANTH674/master/Code/Week3_exer1_solutions.txt)
...
```{r median-solution}
med.hair <- median(as.numeric(hair.color_ordered))

# med.hair gives us the rank (i.e., index) of the median hair color.
# Use it to get R to give us back the hair color
levels(hair.color_ordered)[med.hair]
```
...
We see the answer is `3`, corresponding to `Brown` hair color (can you write soft code so that R returns `Brown` instead of `3`?).

Remember that `median()` has an `na.rm` argument to remove `NA`s if needed.
...
Save to an object a vector of 10,000 normally distributed values with `mean = 0` and `sd = 1` (the default `rnorm()` arguments for `mean` and `sd`). 
...
So we can see that `mean()` is doing its job correctly.
...
Take your object of 10,000 normally distributed values, and let's add one large number to it (i.e., `c(x, 1000000)` if `x` is your object of 10,000 numbers).
...
**NB**: a good general rule is to always use the median when describing the location of non-normal distributions!
...
# log transform & then take the antilog.
# Should get original number back
exp(log(5))
10 ^ log10(5)
2 ^ log2(5)
...
So, to transform the mean of our log-transformed vector back into arithmetic space, we take the antilog of the calculated mean.
...
As with location summary statistics, those describing spread will be different for different data types, so we will go over the various measures of spread according to data type.


## Spread: categorical

For categorical data, we use information measures, though researchers rarely calculate a summary statistic of spread for these types of data.
...
The formula for Shannon's index is $-\sum{p_i \textrm{log} (p_i)}$, where $\sum$ means you sum all the elements in your vector (i.e., with `sum()`), and $p_i$ is the proportion of the $i$th element.
...
The function takes one argument: a vector of numbers, each of which is a count of a factor level.
...
```{r iqr-solution}
IQR(as.numeric(hair.color_ordered))
```

We see the answer is `1`, meaning the center 50% of our ordered data encompasses one "step" between hair colors.
...
As before, let's start with a normally distributed variable.
...
As you did earlier, add 1,000,000 to your vector and then calculate both statistics.
...
```{r non_norm_var-solution}
x <- rlnorm(n = 1000, meanlog = 0, sdlog = 2)

IQR(x)
exp(sd(log(x))) # geometric SD
```
...
This is because the middle 50% of your data comprises only a small range, located at the smaller numbers.
...
Similar to the geometric mean, the geometric SD takes the SD of log-transformed variables and then takes the antilog to bring it back to arithmetic space.
Try it out and see what you get (if you can't figure out the code, click the "Solution" button in the above console).
...
This is important for standardizing variables on different scales (e.g., if two variables were measured in mass and feet, or one variable ranged from 0-5 cm while the other ranged from 500-600 cm). 
Such a standardization transforms data to be in the same unit: standard deviation units (e.g., 1 = one SD unit, 2 = two SD units).
This will be important for principal components analysis and comparing different linear models (i.e., you won't really need to use this transformation until those topics).
...
```{r scale-solution}
x <- rnorm(n = 10000, mean = 500, sd = 20)
hist(x)

x.center <- scale(x, scale = FALSE) # center only
hist(x.center)
mean(x.center); sd(x.center)

x.center.scale <- scale(x) # center AND scale
hist(x.center.scale)
mean(x.center.scale); sd(x.center.scale)
...
For example, $\textrm{log}(x^2)$ becomes $2\textrm{log}(x)$ and $\textrm{log}(2x)$ becomes $\textrm{log}(2) + \textrm{log}(x)$.
...
plot(1:50, log10(pop_size))

plot(1:50, pop_size, log = "y")
```
...
Do this above (using whichever log base you want) and see what happens.
...
I also mentioned three other transformations in lecture that can be used to remove the dependence between the mean and variance of a variable (which is necessary for some statistical tests, but I won't delve into this point any further).
...
This dataset is a vector containing US population sizes (in millions) every 10 years from 1790 to 1970.
    ...
    Plot how US population size changes over time (this vector is in a class called "time series", so you don't need to specify the x-variable, i.e., `plot(uspop)` will work).
...
median(uspop)
IQR(uspop)
# geometric mean & SD work best if the data are lognormal
exp(mean(log(uspop))) # geometric mean
exp(sd(log(uspop))) # geometric standard deviation
hist(log(uspop)) # not quite lognormal though

# Question 2a
# I would treat this as ordinal. 
# Even though distances have meaning, there are just too few categories.

# Question 2b
median(mtcars$carb); IQR(mtcars$carb)

# Question 3a
# The data are continuous.
# Histograms show that they're normal enough to use the mean and SD
...
![](https://www.datanovia.com/en/wp-content/uploads/2020/04/beautiful-plot-in-r-with-summary-statistics-labels-boxplots-with-summary-statistics-1.png){width="100%"}
...
In sum, these are ultimately general rules, and the most important determinant of which kind of plot, summary statistic, and data transformation you should use *depends on your research question and what you want to show*!
Once you clearly define your research question, everything flows from there!
