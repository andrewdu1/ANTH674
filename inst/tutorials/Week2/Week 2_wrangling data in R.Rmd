...
As you will see, there are all kinds of errors and issues in raw data that prevent you from analyzing data easily and efficiently.
...
The `sort()` function creates an output vector where the elements are now sorted in ascending order.
...
**NB:** `length(which(x))` tells you how many `TRUE` are in `x`, which can also be achieved with `sum(x)`. 
...
You can see there are three levels corresponding to the three different species in the `iris` dataset.

In general, when you import data into R, any column with text data will be treated as a factor.
...
How do we standardize all the different iterations of "zebra"?
...
It's up to you whether you want to enter NA into your spreadsheet to indicate missing data.
...
This is good because you always want to be in control of your data and analyses.
...
Now try to subset out one of the character strings with spaces or special characters.
...
Note that you have to use character strings to define variable names with spaces or special characters.
...
```{r mult_data-setup, message = FALSE}
require(RCurl)

mult_data_url <- getURL("https://raw.githubusercontent.com/andrewdu1/ANTH674/master/Datasets/multData_perCell.csv")
...
This is also a poor data practice.

### How the violations affect R
...
```{r mult_data1-setup, message = FALSE, echo = FALSE}
require(RCurl)
...
```{r mult_data1, exercise = TRUE}
```

<div id = "mult_data1-hint">
`mass <- mult_data$mass`
...
The `5; 6` in column `height` can be split using `strsplit()`, though I won't go into the details of how to do that (you can look at the help file and try it out yourself if you'd like).
...
I have created a dataset, `grades`, where the first column is the undergraduate year of the student, and the second column is the students' grades.
Data were input by a non-R-user, so it needs a lot of cleaning!
**NB:** the first column consists of characters, not factors.

1. How many students are there in each year?

2. What's the average grade for the whole class? (it appears some grades were lost; ignore those)

3. What's the average grade for each year? (again, ignore lost grades)

```{r exer2-setup, message = FALSE, echo = FALSE}
require(RCurl)

exer2_url <- getURL("https://raw.githubusercontent.com/andrewdu1/ANTH674/master/Datasets/wk2_tutorial_exer2.csv")

grades <- read.csv(text = exer2_url)
```
...
# standardize each year using grep()
grades$year[grep("freshman", grades$year, ignore.case = TRUE)] <- "freshman"
grades$year[grep("sopho", grades$year, ignore.case = TRUE)] <- "sophomore"
grades$year[grep("junior", grades$year, ignore.case = TRUE)] <- "junior"
grades$year[grep("sen", grades$year, ignore.case = TRUE)] <- "senior"

# subset out grades with "(late)"
late_grades <- grades$grade[grep("late", grades$grade)]
# get ride of "(late)"
late_grades_trim <- substr(late_grades, 1, nchar(late_grades) - 7)
# replace the grades with "(late)" with the trimmed grades
grades$grade[grep("late", grades$grade)] <- late_grades_trim

# There's one question mark grade. NB: when you search question marks with grep(), you have to use "\\?"
grade_ques_mk <- grades$grade[grep("\\?", grades$grade)]
grade_ques_mk_trim <- substr(grade_ques_mk, 1, nchar(grade_ques_mk) - 1)
grades$grade[grep("\\?", grades$grade)] <- grade_ques_mk_trim

# change to numeric
grades$grade <- as.numeric(grades$grade)

# I'm going to remove all rows with NAs for grades
not_NA_grades <- complete.cases(grades)
grades_no_NA <- grades[not_NA_grades, ]

# 1
table(grades$year)

# 2
mean(grades_no_NA$grade)
mean(grades$grade, na.rm = TRUE) # do this if you didn't remove NAs ahead of time

# 3
# use na.rm = TRUE if you didn't remove NAs ahead of time
mean(grades_no_NA$grade[grades_no_NA$year == "freshman"])
mean(grades_no_NA$grade[grades_no_NA$year == "sophomore"])
mean(grades_no_NA$grade[grades_no_NA$year == "junior"])
mean(grades_no_NA$grade[grades_no_NA$year == "senior"])

# a quicker method
tapply(grades_no_NA$grade, grades_no_NA$year, mean) 
# tapply() will run a function (third argument) on a vector (first argument) for elements belonging to each unique value of a character/factor (second argument)
# if you still have NAs in your data, there's an extra step involved
```
...
```{r non_rect-setup, message = FALSE}
require(RCurl)

csv_url <- getURL("https://raw.githubusercontent.com/andrewdu1/ANTH674/master/Datasets/Fig2b_BromanWoo2018.csv")
...
```{r relate_data-setup, message = FALSE}
require(RCurl)

csv_url_iso <- getURL("https://raw.githubusercontent.com/andrewdu1/ANTH674/master/Datasets/Duetal2019_isotopes.csv")
iso <- read.csv(text = csv_url_iso)

csv_url_abund <- getURL("https://raw.githubusercontent.com/andrewdu1/ANTH674/master/Datasets/Duetal2019_abundances.csv")
...
In `iso`, on the other hand, each taxon from each member is associated with a number of enamel isotope measurements. 
...
I'm sure certain sections spoke out to you more, as these are more relevant for your research questions and the datasets you work with (it's best to focus on learning these sections more thoroughly first).
...
Once you know what your research question is and what data and tests you need, it will become immediately clear how you should subset, clean, and organize your data.
