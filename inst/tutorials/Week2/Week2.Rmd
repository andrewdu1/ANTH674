...
2. What's the average grade for the whole class? (it appears some grades were lost; ignore these)
...
# You can click the "Hint" button for the answers when you're done. 
# A design flaw in learnr meant solutions using the "Solutions" button were cut off...
```
...
`tapply()` will run a function (third argument) on a vector (first argument) for elements belonging to each unique value of a character/factor (second argument). If you still have NAs in your data, there's an extra step involved
</div>
