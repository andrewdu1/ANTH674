...
![](https://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/Data_Wrangling_From_Messy_To_Clean_Data_Management.jpg/729px-Data_Wrangling_From_Messy_To_Clean_Data_Management.jpg){width="80%"}
...
There is a learning curve involved, but I would encourage you to learn `dplyr` if you need to do some heavy-duty wrangling or if you work with really large datasets ([here's a nice introduction to `dplyr`](https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html)).
...
You can also reorder an entire data frame in the same way.
...
You can try out `!` above by defining `x <- c(FALSE, TRUE, FALSE)` and then seeing what happens when you enter `!x`.

**Remember, there are many ways to do the same thing in R, so pick the one that works best with how you think!**

### Using `subset()`

`subset()` is a function that can be used to subset a vector or variables within a data frame according to some logical statement.
...
`subset()` is perhaps more useful for subsetting variables (columns) within a data frame.
...
Remember that the first argument is the R object we want to subset (i.e., the `mtcars` data frame), the second argument is the logical statement indicating which elements (or in this case, rows) to keep (i.e., `wt < 3`), and we need a third argument indicating which columns to select (i.e., `mpg`).
...
We can also delete variables from a data frame using `subset()`.
...
We can also use `subset()` to subset out multiple variables (i.e., columns) from a data frame according to some logical statement.
...
The result is a new data frame with just two variables (`mpg` and `hp`) for those car models whose weight (`wt`) is below 3000 lbs.
...
I've simulated this situation for you by creating a new data frame called `mtcars_no_wt`, which has all the original `mtcars` variables except for `wt`. 
I've also created a new vector, `car_wt`, which has all the weights from `mtcars`.

```{r mtcars_no_wt-setup, echo = FALSE}
...
`cbind()` doesn't only have to be used for combining a vector with a data frame, it can also be used to combine multiple data frames or a matrix with a data frame, as long as they all have the same number of rows.
Just for fun, try combining `mtcars` with itself using `cbind(mtcars, mtcars)` and see what happens.

`cbind()` is very flexible and can be used to combine (by column) vectors, matrices, or data frames.
If one starts out with a data frame, however, one can use `$` to add a new variable/column (recall that `$` is used to subset out a variable in a data frame).
For example, I can create a new column in my `mtcars_no_wt` data frame called `weight` with `mtcars_no_wt$weight <- car_wt`.
Try this out above and then enter `mtcars_no_wt` and see what you get.

We can also add rows to a vector, matrix, or data frame using `rbind()`, as long as they all have the same number of columns.
...
1. Create a new data frame called `iris.sepal_setosa` wherein the variables include only `Sepal.Length` and `Sepal.Width` for the `setosa` species only.
Next, sort the rows in `iris.sepal_setosa` by *decreasing* `Sepal.Length`, and save the result to a new data frame called `iris.sepal_setosa_ordered`.

2. Let's create a new vector called `Sepal.Area`, which is equal to `Sepal.Length` times `Sepal.Width`, using data from our new `iris.sepal_setosa_ordered` data frame. Add `Sepal.Area` as a new column to `iris.sepal_setosa_ordered`, and save this new data frame as `iris.sepal_setosa_ordered1`.
...
# this also works
iris.sepal_setosa <- iris[iris$Species == "setosa", c("Sepal.Length", "Sepal.Width")]
# Question 1b. 
...
# this also works
iris.sepal_setosa_ordered1 <- iris.sepal_setosa_ordered
iris.sepal_setosa_ordered1$Sepal.Area <- Sepal.Area
...
Names are also known as **categorical data** in statistics, which are defined as labels assigning each observation to a particular group (e.g., "juvenile" and "adult" when assigning individuals to an age category).
...
Factors can be numbers but are more commonly characters (e.g., `iris$Species` is a factor), and each factor has different **levels** (e.g., `setosa` and `virginica` are two different levels).

You can check that `iris$Species` is a factor by using the `class()` function to see what kind of variable `Species` is: `class(iris$Species)`.
Try this out below.
...
So if you wanted to subset a data frame to include only those data associated with zebras, this can cause problems (you would have to do `species == "zebra" | species == "Zebra" | species == "zebra "`, which is a pain especially if zebra is mistyped in many other ways).
...
```{r zebra-setup, echo = FALSE}
x <- factor(c(rep("zebra", 10), 
              rep("Zebra", 6), 
              rep("zebra ", 2), 
              "zEbra", 
              rep("elephant", 10)))
...
`table()` is another useful function that tabulates and counts unique elements.
Try these functions out on `x` below.
...
A nice way of detecting pesky spaces is to transform your factor levels into characters.
...
That is, the output of `grep()` is the same as `which(grepl())`.

Let's try `grep()` out below.
We want to subset all the different iterations of "zebra" in `x`.
Thus, we begin by figuring out which elements correspond to all iterations of "zebra" by entering `grep("zebra", x, ignore.case = TRUE)`, where the first argument is the pattern to be matched against, the second argument is the vector to be investigated, and `ignore.case` indicates whether pattern matching is case sensitive.
...
You can see that R automatically detected by index all the different iterations of "zebra"!
...
That is `"zebra"` is a subset of `"zebra "`; you can also try the pattern `"zeb"` or `"ebra"`, and you will get the same result.
...
Now it is straightforward to replace all iterations of "zebra" in `x` with one name: `"zebra"`.
...
**An alternative function**: a student from a previous semester preferred the function `replace()`.
This function takes three arguments: the vector whose elements you want to replace, a vector of indexes or `TRUE`/`FALSE` indicating which elements to replace, and the value to replace said elements.
So to replace all iterations of "zebra" with `"zebra"`, one would enter `x <- replace(x, grep("zebra", x, ignore.case = TRUE), "zebra")`.
**Again, use whichever function works best for your brain!**
...
Try to replace `elephant` with `Elephant`, using the code you learned above.
What happens?
...
`x1[grep("elephant", x)] <- "Elephant"`
The `ignore.case` argument in `grep()` isn't necessary here since all `"elephant"` are lower case.

`x1 <- as.factor(x1)`
</div>

R returns a warning message, saying `invalid factor level, NA generated`.
This warning is basically saying that `Elephant` as a factor level is invalid because no such level exists in `x`, so it replaced `elephant` with `NA`s (you can see this by entering `x` after your line of `grep()` code).
Not good (this is also why it's a good idea to save modified vectors as new ones, e.g., `x1`, in case something goes wrong)!

To create new factor levels, we have to first transform `x` into a vector of character strings (do you remember how to do that? The answer is in the "Hint" button).
Now, we can replace `"elephant"` with `"Elephant"`.
Try this out above.

To transform our vector back into a factor, we have to use `as.factor()`.


## Missing data

Missing data in R is represented using `NA`s.
If there are blank cells in your spreadsheet (actual blanks, not cells with a space!), R will automatically transform them into `NA`s when you import the dataset.
...
For example, below is a data frame, called `DF`, with two columns of numbers, some of which have `NA`s.
...
`is.na()` is a function that returns a logical vector, matrix, or data frame (i.e., the format of the object you're investigating), where elements are `TRUE` if they are `NA`.
Try it out on `DF` above.

Therefore, `sum(is.na(DF))` will tell you how many `NA`s are in the data frame.
...
Thus, to remove `NA`s from the data frame `DF`, you would enter `DF[complete.cases(DF), ]`.
...
This `NA` result is good because you always want to be in control of your data and analyses.
That is, you don't want R using a function on data that has `NA`s you don't know about.
It's better for R to return `NA` as the default, so you know there are `NA`s and can find out where they are and why they're there.
Once you're satisfied with your knowledge of the `NA`s, you can then actively ignore them in functions by setting `na.rm = TRUE`. 
...
Let's see what happens when you create a data frame, wherein variable names have a space or special character in them.
...
Use `colnames(DF)` to see what R does to spaces and special characters in variables names.

```{r variab, exercise = TRUE}
DF <- data.frame("mass(kg)" = 1:10, 
                 "upper case" = LETTERS[1:10], 
                 lower_case = letters[1:10])
```
...
Try out other different special characters to see what happens (they usually get transformed into periods).
...
Because of this, **it's just easier to not use spaces or special characters in names at all!**
...
In R, each column within a data frame can be a different class (e.g., numeric, character, factor), but all elements *within* a column must be the same class.
Let's see what happens when we import the above spreadsheet, where this is *not* the case.

The dataset is in a data frame called `mult_data`.
Use `head(mult_data)` to look at the first six rows of the data frame (i.e., get an idea of the data frame's structure).
...
#### How I learned to do it in grad school
...
`nchar()` tells us how many letters are in `"andrew"`, so we would extract our desired letters by entering `substr("andrew", 1, nchar("andrew") - 1)` (read the `substr()` help file to understand the arguments).
Try it out below.
...
Instead of using `substr()` and `nchar()`, the `gsub()` function will replace some specified existing partial string by another specified partial string.
For example, imagine I want to trim `" kg"` from the end of `"12 kg"`.
I can use `gsub()` to tell R to replace `" kg"` with `""`.
Assigning `"12 kg"` to `x`, i.e., `x <- "12 kg"`, `gsub(" kg", "", x)` will trim off `" kg"` and return `"12"`. 
...
Calling the data frame `d`, the code would be `subset(d, strain == "A" & genotype == "normal", response)` or `d$response[d$strain == "A" & d$genotype == "normal"]`.
...
For example, I've loaded for you two published datasets from [Du et al. (2019)](https://www.sciencedirect.com/science/article/pii/S0034666718301234).
...
`iso` is a data frame with stable carbon isotope measurements from fossil tooth enamel and ancient soil carbonates for geological members in eastern Africa.
`abund` is a data frame with abundances of taxa whose teeth were analyzed from these members.
...
*Exactly* means the classes match up too, so if you compare two vectors that contain the same exact names but one is a factor and the other is a character, `identical()` will return `FALSE`.
As a quick example, you can enter `identical(as.character("A"), as.factor("A"))` and see what happens.

You can see how it would be difficult to combine `iso` and `abund` into one dataset that is still rectangular.
...
But because the datasets are linked by shared taxon and member names, it is straightforward to link and analyze them together.
...
![](https://askananthropologist.asu.edu/sites/default/files/styles/aab-gallery-large/public/gallery-images/hadar_camp.jpg?itok=QpbV1OYI){width="70%"}
...
(I have kept the datasets loaded for you; click "Hint" if you're stuck)
...
`iso$d13C[iso$data.type == "enamel" & iso$taxon == "Aepycerotini" & iso$member == "SidiHakoma"]`

`abund["Aepycerotini", "SidiHakoma"]`
or
`abund$SidiHakoma[rownames(abund) == "Aepycerotini"]`
</div>

Now subset out the abundance for this taxon in this member (using the `abund` data frame).

Now we can do whatever analyses or visualization we'd like with these two pieces of related data!
