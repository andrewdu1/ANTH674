...
![](https://i0.wp.com/r-exercises.com/wp-content/uploads/2017/01/fancy-rgl-1-300x300.png?zoom=2.640000104904175&resize=300%2C300){width="50%"}
...
where $Y$ is the DV, $X_{1..n}$ are the different IVs, $\beta_0$ is the intercept, $\beta_{1...n}$ are the slope estimates for each IV, and $\epsilon$ is the error term or residual.
...
One can see why this would be of interest: we often want to isolate and understand the effect of a single IV on the DV.
In fact, researchers will often include an IV that they're not interested, just so they can remove its effect when interpreting other multiple regression coefficients (these "uninteresting" IVs are called **covariates**).
...
Another very similar way to think about, for example, the `drat` coefficient is by removing or **partialing** out the effect of `hp` (this is also why multiple regression coefficients are often called **partial regression coefficients**).
...
Do this below, and then regress the residuals from `qsec ~ hp` onto the residuals from `drat ~ hp`.
...
One might think that adding more IVs to your multiple regression model is a good thing, since you can account for more IVs when studying your IV(s) of interest and $R^2$ increases.
...
This is especially true as your your ratio of data points to parameters decreases (rule of thumb is you should have 20 data points per parameter).
Overfitting is bad since you're not describing the signal and pattern of interest anymore (i.e., how your DV changes with some IV).
Overfitting is extremely bad when you want to predict DVs for a new dataset, since the noise structure in the new dataset will not match what is in your training data.
...
In this example, an interaction is needed between training time and equipment equality.

So in brief, an interaction term indicates the slope between $Y$ and $X_1$, for example, changes as a function of $X_2$.
...
A positive interaction term means the slope of $Y$ as a function of $X_1$ *increases* as $X_2$ increases.
A negative interaction term means the slope of $Y$ as a function of $X_1$ *decreases* as $X_2$ increases.
...
I have also created a function for you in the console (`slope_x1()`) to get out the slope of `x1` for a given value of `x2`; the two arguments are the fitted interaction model and a vector of values for `x2`.
So, use `slope_x1()` to get a vector of expected `x1` slopes for the interaction model and a vector of `x2` values.
...
This can either produce an accelerating curve (see below) or a horseshoe-shaped curve, where a negative slope decreases until it hits zero and then becomes positive with increasing magnitude.
...
If the quadratic term is negative, then the slope of $Y$ as a function of $X$ *decreases* as $X$ increases, producing either a decelerating curve or an upside-down horseshoe.
...
The `I()` tells R that `^` is a raised-to-the-power operator in the formula.
See what happens when you fit a linear model without `I()` (compare it to `y ~ x`).

Go ahead and fit the quadratic model.
To add the regression line to the scatter plot, we cannot use `abline()` anymore, which only works for linear models with two coefficients (i.e., the intercept and slope in simple linear regression).
We now have to use the `curve()` and `predict()`, so calling the fitted quadratic model `lm.quad`, the proper code is `curve(predict(lm.quad, data.frame(x = x)), add = TRUE)`.
...
Note that the residuals show a horseshoe shape, indicating the relationship between the DV and IVs is not linear.

Now include the interaction term and create the diagnostic plots.
The residuals look a lot better now, demonstrating the model is specified correctly and no assumptions are violated.

You can even see what the estimated coefficients are and how well they match what we simulated: intercept = 0, `x1` coefficient = 1, `x2` coefficient = 1.5, and interaction term = 2.
...
Do the residuals indicate the model is well-specified (i.e., no assumptions are violated)?
...
You can see that the `hp` coefficient did not change too much between the models, but the P-value increased by two orders of magnitude.
...
If a coefficient in multiple regression is defined as the change in DV as IV increases by one, holding all other IVs constant, the model is having trouble holding other IVs constant given that at least one is collinear.
...
If your IVs are 100% unrelated, then the multiple regression coefficients will be identical to their counterparts in simple linear regression.
...
Calculate the VIF for `wt` in the `lm(qsec ~ hp + wt)` model (there is also a `vif()` function in the `car` package).
Note that a rule of thumb is a VIF > 10 indicates that your IV is collinear with others.
...
You can see that the VIF is actually quite low (1.77)!
This is an example where statistical nuance trumps rules of thumb.
...
Moreover, the results are unaffected by collinearity, and in fact, they can tell us how collinear variables are.
...
Then, we need to get the $R^2$ from the simple regressions for each IV.
...
![](https://www.researchgate.net/profile/Tiffany_Schriever/publication/272787302/figure/fig4/AS:294680144171021@1447268629298/Variance-partitioning-R-2-for-trait-composition-A-and-taxonomic-composition-B-of_W640.jpg){width="70%"}
...
lm.x1 <- lm(y ~ x1, data = DF) # x1 changes by a lot
...
All of this material will served as the foundation for next week's lecture, where we will learn about general linear models with non-continuous IVs and DVs (e.g., ANOVA, logistic regression).
