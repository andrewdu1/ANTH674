...
With LMMs, one can do **variance partitioning** and see how much DV variation is attributable to the fixed effects, random effects, and residual variation.
Perhaps more commonly, one can fit an intercept-only LMM and see how the DV variation is partitioned among multiple random effects.
...
A nested random effects LMM can be fit with `lmer()` using `lmer(strength ~ 1 + (1 | batch) + (1 | batch:cask), data = Pastes)`.
You can see that the larger, encompassing factor is specified first with `(1 | batch)`, which is followed by `(1 | batch:cask)`, wherein the nested structure of `cask` within `batch` is denoted by the ordering of the factors in `batch:cask` (i.e., larger factor first, smaller nested one second). 
You can also specify the nesting structure with `batch / cask`, so this also works: `lmer(strength ~ 1 + (1 | batch / cask), data = Pastes)`.
Lastly, you can also do nested random effects using `lme()`: `lme(strength ~ 1, random = ~1 | batch / cask, data = Pastes)`.
...
```{r nest-solution}
lmm.res <- lmer(strength ~ 1 + (1 | batch) + (1 | batch:cask), data = Pastes)
# or
lmm.res <- lmer(strength ~ 1 + (1 | batch / cask), data = Pastes)

as.data.frame(VarCorr(lmm.res))$vcov


lmm.res1 <- lme(strength ~ 1, random = ~1 | batch / cask, data = Pastes)
...
As you can see, most of the variance in `strength` is among casks (`cask`), rather than among `batch` or within `cask` (i.e., between `sample`).
...
# Q2
nurses_nest <- lmer(stress ~ 1 + (1 | hospital) + (1 | hospital:wardid), data = nurses)
