...
# **Week 11:** Multivariate models & principal components analysis
...
2. Condensing multiple variables into fewer important composite variables for visualization purposes (i.e., **ordination**).
...
anova(manova(cbind(d$DV1, d$DV2) ~ d$grp)) # also works if you use cbind() to create the matrix
```
...
`manova()` uses formula notation, where the DV must be a matrix, whose columns are the individual variables.
...
First, run two separate one-way ANOVAs, each testing whether student satisfaction or test scores are significantly different across teaching methods (you can create boxplots to visualize the data).
...
anova(manova(cbind(teach$Test, teach$Satisfaction) ~ teach$Method)) # also works
```
...
Ordination allows us to get around this problem by distilling multiple variables into a smaller number of important composite variables.
...
Conceptually, it creates composite variables (i.e., **principal components** or PCs) by first centering the data and then fitting a line through the axis of greatest variation (see animation below).
It then rotates the plot so this line (i.e., the first principal component or PC1) is now on the x-axis.
Subsequent PCs are fit so that they're perpendicular (and thus independent) to previous PCs.
...
Because PCA works by fitting PCs through axes of greatest variation, more variable variables will strongly influence how PCs are fit to the data.
...
This is known as a **scree plot** (see image below), which mimics the profile of a mountainside where rubble has fallen (i.e., a scree slope).
...
In this context, eigenvectors are also called **loadings** because they tell us how each of the original variables "loads" on each PC.
...
sum(plant1 * iris.pca$rotation[, "PC1"]) # compare to iris.pca$x[1, 1]
```
...
It is better to fit one's analysis to the research question instead of the other way around (the research question is king!).
...
This week, we only focused on those multivariate models that have one categorical IV (i.e., Hotelling's *T*^2^ and MANOVA).
