...
![](https://upload.wikimedia.org/wikipedia/commons/thumb/c/ce/MNP_-_Turkanajunge_1.jpg/800px-MNP_-_Turkanajunge_1.jpg){width="50%"}

There is certainly a statistical population of all *Homo erectus* individuals who have ever lived, but the fossil record only preserves some individuals, and we can estimate stature for only some of those individuals.
...
![](https://psmag.com/.image/c_limit%2Ccs_srgb%2Cq_auto:good%2Cw_380/MTMzNTIwNTUwOTIzMzg4OTM4/shutterstock_109028771jpg.webp){width="40%"}

A coin flip can either come up heads or tails.
With two outcomes, this is a binomial random variable.
...
You might have gotten 0, 1, or 2 because there is randomness (i.e., ***stochasticity***) involved in flipping coins.
...
If you make a histogram of your results, this is the ***sampling distribution*** of number of heads flipped in two trials (more on sampling distributions in *Frequentism using measurements*).
...
plot(1:length(prop_heads), prop_heads / 1:length(prop_heads), type = "l", log = "x", xlab = "Number of coin flips", ylab = "Proportion that are heads")
...
plot(x, dnorm(x, mean = 5.5, sd = 0.3), type = "l", xlab = "Human height (feet)", ylab = "Density")
```

```{r norm, exercise = TRUE}
```

Let's take a random sample of 10 individuals from this population, using `rnorm(10, 5.5, 0.3)`.
...
```{r eval = FALSE}
rbinom10 <- array(dim = 10) # empty vector of length 10 called rbinom10

for(i in 1:10){ # iterating through 1 to 10, with each number assigned to i each time. Even better would be to type length(rbinom10) instead of 10

  rbinom10[i] <- rbinom(1, 2, 0.5) # for each iteration where i = 1, 2, 3, ...10, generate number of heads from two coin flips and save the result to the ith empty element in rbinom10
}
```
...
It is *NOT* a distribution of the number of observations in your sample.
...
Each data point in the distribution of observations from a sample is just that: an observation; to plot this, you would enter `hist(rnorm(100, mean = 5.5, sd = 0.3))` to get a sample of 100 observations from the statistical population.
Do this and compare it to the histogram of the sampling distribution of means.
The latter has smaller spread because the data points are means, each of which is distilling their respective original distribution of observations into its central tendency.
...
plot(x, dlnorm(x, meanlog = 1.5, sdlog = 0.5), xlab = "", ylab = "Density", type = "l")
```
...
It's normal!
This is the ***central limit theorem*** (CLT): no matter the shape of the underlying population distribution, the sampling distribution of means (or sums) will be normally distributed with a large enough sample size (in our case, 100).
...
The mean of a sampling distribution provide us with an estimate of a population parameter (e.g., population mean).
...
Note that we can use the mean and SD because the CLT guarantees our sampling distribution of a mean or sum will be normal!
...
![](https://sophialearning.s3.amazonaws.com/markup_pictures/9562/68-95-99.7.png){width="40%"}
...
If so, 6 feet would be an unlikely value, given our sampling distribution (you can see how likely 6 feet is in the `norm_samp_dist` sampling distribution).
...
Monte Carlo is a class of methods that use repeated random sampling to obtain numerical results.
Monte Carlo is the name of the famous gambling resort city on the Riviera. 

![](https://miro.medium.com/max/750/1*DfT4vHYNWrtxFAp7lendWg.png){width="40%"}
...
Let's once again create a random sample of 100 observations from our population (`mean = 5.5, sd = 0.3`).
I will present each coding step involved in bootstrapping the sampling distribution for the population mean.

1. Create an empty vector of length 1,000 since we will do 1,000 bootstrap iterations (feel free to do more if your computer is fast enough!).
2. Initiate a `for()` loop, where `i` goes from `1` to length of the empty vector.
3. Within the curly braces, resample with replacement the vector of 100 observations; you can save this to a new object.
4. Calculate the mean of the object, and save the result to the $i$th element of your empty vector.

Try this out below.

```{r boot, exercise = TRUE}
# Click "Solutions" button if you get stuck
```

```{r boot-solution}
x <- rnorm(100, 5.5, 0.3)

boot_dist <- array(dim = 1000)

for(i in 1:length(boot_dist)){
  
  x_boot <- sample(x, replace = TRUE)
  
  boot_dist[i] <- mean(x_boot)
}
```
...
sepal.length_ci <- c(sepal.length_mean - 1.96 * sepal.length_se, sepal.length_mean + 1.96 * sepal.length_se)
```
...
In frequentist statistics, the parameter value is fixed, and the confidence intervals move around depending on the samples drawn from the population (in Bayesian inference, it's the complete opposite!).
...
With 50 CI, on average 2.5 will miss the true mean (i.e., 0), so we're expecting 2 or 3 CI to miss (though you might get 4 or none!).

I will tell you how to code this step by step.

1. Create an empty array, called `ci.res`, with 50 rows and 2 columns (rows for 50 CI and one column each for the lower and upper CI limit, respectively).
2. Use a `for()` loop to iterate through each row, i.e., `i in 1:nrow(ci.res)`.
3. Save to an object, `x` 100 observations from the population with `mean = 0, sd = 1`.
This is done with `rnorm(100)`.
...
plot(1, type = "n", xlim = c(1, nrow(ci.res)), ylim = range(ci.res), main = "50 replicates", xlab = "Replicate number", ylab = "Value")
segments(1:nrow(ci.res), y0 = ci.res[, 1], y1=ci.res[, 2], lwd = 2)
abline(h = 0, col = "red")
```

How many CI missed the true value, i.e., 0?

So, let's say you calculate 95% CI on a sample of data you're analyzing.
Your calculated CI is equivalent to *one* of the CIs calculated above.
...
Confidence intervals shrink as sample size increases because the law of large numbers tells us that sample means are estimated more precisely with larger sample sizes.
...
We will create CI using samples of different sizes that are drawn from a normal distribution with `mean = 0, sd = 1`.
...
plot(1, type = "n", xlim = range(n), ylim = range(ci.n), main = "", xlab = "Sample size", ylab = "Value", log = "x")
segments(n, y0 = ci.n[,1], y1 = ci.n[,2], lwd = 2)
abline(h = 0, col = "red")
```

So, the takeaway here is that as sample size increases, the CI will shrink around the true value.
As a result, you can be pretty confident that you know what the true parameter value is, which is great if you want to use the data to corroborate some predicted value from a hypothesis.
...
This means we know the null hypothesis will be falsified even before we conduct the test.
...
Now, we will use bootstrapping to calculate a P-value, testing the null hypothesis that there is no difference between the means of both groups' populations.
...
![](https://www.cliffsnotes.com/assets/267184.png){width="40%"}
...
Why is this?
This is because our bootstrapped iterations randomly assigned heights (with replacement) to new samples of `x` and `y`, such that sometimes `mean(x)` was larger and sometimes `mean(y)` was larger.
Because we're not concerned with which group is larger, we need to consider the null distribution values in either side of the tail.
...
Go ahead and calculate a two-tailed P-value.
...
```{r exer3-solution}
obs_diff <- mean(sleep$extra[sleep$group == 1]) - mean(sleep$extra[sleep$group == 2]) # note that group 2's mean is larger
...
sum(abs(boot_diff) >= abs(obs_diff)) / length(boot_diff) # using abs() is a nice way to get rid of negatives to calculate a two-tailed p-value
...
You can see that these mean differences are unusually large, even though the true mean difference is zero.
...
That's why my philosophy is to calculate as few P-values as possible (more on this next week).

BUT, the null hypothesis is almost *never* true, and that leads us to our next topic.
...
Basically, these three things act to decrease the overlap between the distributions of two samples, such that their mean differences are more obvious.
...
This is due to the law of large numbers: a larger sample size ensures that sample means of groups are estimated so precisely that their differences will be clear, and the sampling distribution of mean differences will have very narrow spread (such that a small observed mean difference will fall outside the sampling distribution and be significant).
...
I will write a function for you that does this (see if you can understand what I did).

```{r}
pvals_n <- function(n, n.iter, mean1, sd1 = 1, mean2, sd2 = 1, PLOT = TRUE){
  ...
  for(i in 1:nrow(p_res)){ # go through each iteration
    ...
    for(j in 1:ncol(p_res)){ # go through each sample size
      ...
      p_res[i, j] <- t.test(rnorm(n[j], mean = mean1, sd = sd1), rnorm(n[j], mean = mean2, sd = sd2))$p.value # calculate t-test and save p-value
    }
  }
  ...
  if(PLOT){
    ...
    plot(n, p_plot, log = "x", type = "l", lwd = 3, xlab = "Sample size", ylab = paste("P-value averaged over", n.iter, "iterations"), ylim = c(0, max(p_plot)))
...
Copy and paste the function into the console below.
...
You can see how even a small mean difference will be exploited as significant if sample size is large enough.
...
We even learned how you can approximate the sampling distribution from just one sample (this was necessary before powerful computers and bootstrapping)!
