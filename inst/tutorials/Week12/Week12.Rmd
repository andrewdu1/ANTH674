...
# **Week 12:** Ordination continued & cluster analysis
...
We first learned about **distance matrices**, which are the foundation for most, if not all, multivariate methods.
...
**Distance matrices** are matrices that summarize how (dis)similar variables are when compared in a pairwise fashion.
...
If we wanted to create a distance matrix comparing each pair of petal and sepal measurements to each other, we would have to transpose the dataset first, using `t()`.
...
# Click "Solution" button if stuck (exercise below)
```
...
They should have a set number of rows and columns (which should be equal), but `dim()` does not work on them. 
...
This makes sense as the distance for Plant 1 and 2 is the same as that for Plant 2 and 1.
...
You can click the "Solution" button if you get stuck.
...
Some common ones are Jaccard and Sorensen for variables with binary outcomes (e.g., 0s and 1s), chi-square and Bray-Curtis for frequencies of categorical data, and Gower for mixed continuous and categorical data (see pg. 404 in your textbook for more distance metrics, though there are even more than that!).
...
This distance matrix is telling us how similar hair colors are to each other, in terms of the proportion of individuals in each eye color.
...
To fix my flipped y-axis, I simply multiply all principal coordinate 2 scores by -1: `iris_pcoa$points[, 2] <- -1 * iris_pcoa$points[, 2]`. 
...
We are now extracting three principal coordinates with `k = nrow(haireye) - 1`, the maximum allowed given that there are only four rows (for $n$ rows, you can only get $n-1$ principal coordinates).

Create the PCoA plot.
In lecture, I used `text()` instead of `points()` to add the hair color names to the plot.
Look at the help file to see if you can figure out how to do it on your own (click the "Solution" button if you get stuck).
...
Try that out above (click the "Solution" button if you get stuck).
...
You can see that both soil core (blue circles) and mite species (red triangles) are automatically plotted, and percent variance explained is automatically calculated and added to the plot.
...
As you can see, black hair is far away from brown, as to be expected given how different their proportion of eye colors are (again, enter `mosaic(haireye, shade = TRUE, legend = TRUE)` to visualize this).
...
A lower stress is preferred, where any number below 0.2 is acceptable (rule of thumb).
The stress of our NMDS analysis is pretty much zero indicating that there's a new perfect correlation between the Euclidean distances of NMDS points and the chi-square distances.
...
For your lecture, I chose a random starting configuration, hence why the scatter plot and Shepard diagram looks different.
...
![](https://static.packt-cdn.com/products/9781789952292/graphics/C12626_02_15.jpg){width="70%"}
...
There are some subtle differences with the chi-square distance at which higher clusters are joined.
...
Then create a two-dimensional NMDS plot of the sites.
...
Ordination plots multivariate data in a reduced dimensional space, with closer points reflecting observation with more similar variables.
...
These two method are just different ways of visualizing distance matrices; whichever one is more appropriate is dicatated by your research question.
