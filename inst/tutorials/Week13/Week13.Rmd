...
# **Week 13:** Probability, likelihood, & Bayesian methods
...
`dbinom()` gives us the probability resulting from a given value(s) of $X$, $n$, and $p$ for the binomial PMF.
...
Instead of enumerating all possible outcomes in the sample space, which becomes very tedious with more coin flips, these probabilities can be obtained by inserting the number of heads of interest ($X$) into the equation above along with $n=3$ and $p=0.5$.
...
x <- 0:3
plot(x, dbinom(x, 3, prob = 0.5), type = "h", lwd = 3, xlab = "Number of heads", ylab = "Probability", main = "Binomial PMF; n = 3, p = 0.5")
```
...
x <- 0:10
plot(x, dpois(x, 2), type="h", xlab = "Number of phone calls", ylab = "Probability", lwd = 3, main = expression(paste("Poisson PMF; ", lambda, " = 2")))
```
...
plot(x, dnorm(x, mean = 0, sd = 1), type = "l", ylab = "Probability", lwd = 3, main = "Normal PDF; mean = 0, SD = 1")
```
...
As the lognormal's measure of dispersion approaches zero, its shape becomes normal (e.g., compare `hist(rlnorm(1000))` to `hist(rlnorm(1000, sd = 1e-10))`).
...
plot(x, dlnorm(x, meanlog = 0, sdlog = 0.5), type = "l", lwd = 2, col = "red", ylab = "Probability", main = "Lognormal PDF")
...
```{r echo = FALSE, fig.cap = "**Figure 9.** The left plot shows two distributions, and we want to calculate a one-tailed P-value to see if the mean of the red group is significantly greater than the mean of the blue group. The right plot shows the sampling distribution of the test statistic (i.e., the t-statistic), assuming the null hypothesis is true (i.e., there is no difference between the groups). The P-value is equivalent to the probability of getting a test statistic greater than or equal to that observed (0.81), and this is equal to the gray-shaded area under the curve. One can calculate the area using the CDF for the t-distribtion."}
...
Let's do this in the console below (I have simulated the two groups for you: `red` and `blue`).
...
# Click "Solutions" button when done
```
...
For example, P-values calculate the probability of getting the data (or more extreme), assuming the true parameter value is zero (i.e., the null hypothesis).
Likelihood flips this conditional probability and is interested in the probability of the model, given the data: $P(M|D)$.
...
Traditional inference, on the other hand, first assumes some parameter value (usually the null value of zero), assesses how probable the observed data are under this null value, and then makes an inferential statement about the parameter (a very circuitous form of inference!).
...
plot(x, dbinom(x = x, size = n, prob = X / n), type = "h", lwd = 2, xlab = "X (number of successful stalks)", ylab = "P(X | p)", main = "Probability (assuming p = 0.22)")

plot(p, dbinom(x = X, size = n, prob = p), type = "l", lwd = 2, xlab = "p (stalking success rate)", ylab = "L(p | X)", main = "Likelihood function")
```
...
The violation of the first axiom is OK, since we're only concerned with which parameter values have higher likelihoods relative to others.
...
plot(p, dbinom(x = X, size = n, prob = p, log = TRUE), type = "l", lwd = 2, xlab = "p (stalking success rate)", ylab = "logL(p | X)", main = "Log-likelihood function")
```
...
LRTs ask if the extra free parameter increases the likelihood above and beyond any difference expected from sampling error (i.e., the parameter is significant).
...
Similar to before, we work with log-likelihoods, so the log-transformed likelihood ratio becomes the difference between two log-likelihoods: $log(L(M_0)/L(M_1)) = log(L(M_0)) - log(L(M_1))$.
...
As I mentioned in our very first lecture, a powerful form of scientific inference is to compare models on equal footing, as opposed to privileging one model over another (null hypothesis tests privilege the null hypothesis by requiring a very low P-value for rejection).
...
AIC thus accordingly balances goodness of fit (i.e., the log-likelihood) with model complexity (i.e., the number of free parameters): $AIC = -2logL + 2K$, where $K$ is the number of free parameters in the model.
...
The formula for this is $AIC_c=AIC + (2K[K+1])/(n-K-1)$.
...
A rule of thumb is that an AIC or AIC~c~ difference of more than two means strong model support, so M~1~ has much stronger support than M~0~.
...
![](https://patch.com/img/cdn20/users/22871539/20190308/092626/styles/raw/public/processed_images/53739858_400115654125174_787329627763769344_n-1552053921-7468.jpg){width="40%"}
...
![](https://upload.wikimedia.org/wikipedia/commons/thumb/f/f3/Beta_distribution_pdf.svg/1200px-Beta_distribution_pdf.svg.png){width="50%"}
...
plot(p, dbeta(p, shape1 = 2, shape2 = 3), type = "l", lwd = 2, ylab = "Probability", main = "Prior distribution")
```
...
plot(p, dbinom(x = 0, size = 40, prob = p), type = "l", lwd = 2, ylab = "L(p | X)", main = "Likelihood function")
```

From the plot of the likelihood function, you can probably guess that the $MLE$ of $p$ is zero.
...
Because our prior distribution is not concentrated on zero, this ensures that the final estimate of $p$ will be greater than zero, even though the data are concentrated at zero.
...
This is OK because we only care about which values of $p$ are more probable relative to each other.
...
plot(p, posterior, type = "l", lwd = 2, ylab = "P(H|D)", main = "Posterior distribution")
```
...
Note that the posterior PDF is itself a beta distribution with new parameters $\alpha_1 = \alpha + X$ and $\beta_1 = \beta + n - X$.
...
plot(n, post.mean, type = "l", lwd = 2, xlab = "Sample size", ylab = "Posterior mean")
abline(h = a / (a + b), col = "red", lwd = 2)
```
