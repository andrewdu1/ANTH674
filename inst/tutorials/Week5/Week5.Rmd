...
Last week, we went over the foundations of inferential statistics in a frequentist framework.
...
In statistics, we often say that the dependent variable varies *as a function of* the independent variable.
...
![](https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Linear_regression.svg/1280px-Linear_regression.svg.png){width="60%"}

We will cover what linear regression is used for, what it tells us, and how it's used in statistical inference.
Everything you learn in this tutorial is *incredibly* important, as it forms the foundation for future lectures (e.g., multiple regression, GLMs, generalized linear models, mixed models, principal components analysis).
...
#### 2) Learn how to use the `lm()` function to fit a linear regression model to data and how to interpret the outputs.
...
1. **Exploration**: just want to know what the estimated intercept and slope are
...
First, recall that a simple linear regression model fit to data consists of three parts:
...
![](https://nextjournal.com/data/QmfPuPp4V74FyvTTojMj6ix9T8Skj1ji4GhX5Pr6zK8w4N?filename=linear-regression.png&content-type=image/png){width="70%"}
...
Because the independent variable's values don't depend on another variable, we can just simulate whatever we want.
...
y <- 10 + 5 * x + rnorm(30, sd = 5) 
y <- 1.5 + 0.5 * x # no error terms
```

We see that we have a nice linear relationship because we simulated our data to be this way (following the above equation)!
Now let's fit a linear regression to the data, using **ordinary least squares**.
...
What do you think will happen if you increase SD (i.e., the `sd =` argument in `rnorm()`)?
...
We then fit the linear regression each time, saving the intercepts and slopes.
We finally average over all iterations, and the resulting mean intercept and slope should be very close to their true values.
(**NB:** Using `for()` loops in this way is a great way to assess how accurate and precise an estimator is!)

Let's try this below, using the code from before for simulating data (see if you can write the code yourself, using the "Solution" button as a "last resort").
...
3. Randomly sample the independent variable from its population with `x <- runif(30, min = 0, max = 10)`.
4. Randomly sample the dependent variable from its population with `y <- 2 + 3 * x + rnorm(30)`.
5. Fit the linear regression with `lm(y ~ x)`, and save the results to `lm.res`.
6. Call up the coefficients using `lm.res$coefficients`, and save them to the $i$th row of your empty array.
...
We can now see that the intercept and slope are on average *extremely* close to their true values (i.e., 2 and 3, respectively)!
...
# brings up coefficients
lm.res$coefficients

# brings up fitted values
lm.res$fitted.values
# same as
lm.res$coefficients["(Intercept)"] + lm.res$coefficients["x"] * x

plot(x, y)
abline(lm.res)
points(x, lm.res$fitted.values, col = "red")
```
...
`lm.res$fitted.values` returns a vector of DV values predicted from the regression for each IV value.
...
Then enter `points(x, lm.res$fitted.values, col = "red")` (just use a color that's different from your other points and line).
...
If you plot the IV and DV with the estimated regression line, you can see which points are far above the line (large positive residuals), which are far below (large negative residuals), and which are close to the line (small residuals).
...
What do you think you'll get if you enter `points(x, lm.res$fitted.values + lm.res$residuals, col = "blue", cex = 2)`.
...
points(x, lm.res$fitted.values + lm.res$residuals, col = "blue", cex = 2)
```
...
(You square these numbers to get rid of negatives because you're interested in the magnitude only)
...
Try it out by setting `sd =` in `rnorm()` to more than 1.
...
You have to use the variable name and not `$` because this is an array, not a data frame.
...
The first argument is your linear regression object (e.g., `lm.res`), the second argument is a data frame with new values for your IV assigned to your IV object name, and the third argument is `interval = "predict"` if you want to create a 95% prediction interval for your estimates.
...
```{r predict-solution}
predict(lm.res, data.frame(x = c(0.5, 15)), interval = "predict")
```
...
lm.res <- lm(mpg ~ wt, data = mtcars)
lm.res$coefficients

lm.res.center <- lm(mpg ~ scale(wt, scale = FALSE), data = mtcars)
lm.res.center$coefficients
```

You can see that this intercept is somewhat nonsensical: 37.3 mpg is larger than any mpg in our dataset (max. = `r max(mtcars$mpg)`) because no car is zero pounds (which itself is a ludicrous idea).
...
lm.hp.scale <- lm(qsec ~ scale(hp), data = mtcars)
lm.wt.scale <- lm(qsec ~ scale(wt), data = mtcars)

lm.hp.scale$coefficients
lm.wt.scale$coefficients
```

We now see an issue: `hp` ranges from `r min(mtcars$hp)` to `r max(mtcars$hp)`, while `wt` ranges from `r min(mtcars$wt)` to `r max(mtcars$wt)`.
...
The IV corresponding to the larger slope is the one with the larger effect on determining speed (`qsec`).
...
Note that these two variables exhibit a linear relationship and do not need any log-transformations; we are doing so only to illustrate how the slope should be interpreted with log-transformed variables.
...
```{r log_dv-solution}
exp(lm.res$coefficients["x"])

dv.pred <- exp(predict(lm.res, data.frame(x = 1:2)))
...
You'll see that the answer is exactly the antilog of the slope (i.e., the proportional change in unlogged DV as IV increased by one)!

#### Log-transforming the IV only

In lecture, I said that a 1% increase in the unlogged IV translates to a change in the DV that is equal to the slope divided by 100 (this applies only to natural log-transformed IVs).
I will leave that for you to assess on your own.

Let's instead look at the exact case where a Z% increase in unlogged IV translates to a change in the DV that is equal to the slope times log(1.Z) (applies to natural log only).
...
```{r log_iv-solution}
lm.res$coefficients["log(x)"] * log(1.1)

diff(predict(lm.res, data.frame(x = c(1, 1.1))))
```
...
In lecture, I said that if both the IV and DV are natural log-transformed, the slope is interpreted as the approximate percentage change in unlogged DV as unlogged IV increases by 1%.
...
```{r log_iv_dv-solution}
lm.res$coefficients["log(x)"]

pred.dv <- exp(predict(lm.res, data.frame(x = c(1, 1.01))))

pred.dv[2] / pred.dv[1]
```

I got a slope of 0.86, which means that the unlogged DV is expected to increase by 0.86% as unlogged IV increases by 1%.
Let's actually increase the IV by 1% in arithmetic space and see how much the corresponding DV values increase: `predict(lm.res, data.frame(x = c(1, 1.01)))`.
Take the antilog of the DV values with `exp()` and divide the second element by the first.
...
I will do this using some simple algebra, but if that intimidates you, you can skim the equations and proceed to the graphical demonstrations.
All this emphasizes how one can use transformations to linearize relationships between variables so that you can use linear methods (e.g., linear regression).
...
\begin{equation}
\begin{aligned}
  \text{log}(Y) &= \text{log}(ab^X)
  \\
  \text{log}(Y) &= \text{log}(a) + \text{log}(b^X)
  \\
  \text{log}(Y) &= \text{log}(a) + \text{log}(b)X
\end{aligned}
\end{equation}

This equation is now a linear function with a log-transformed DV, and unlogged IV, an intercept equal to $\text{log}(a)$, and a slope equal to $\text{log}(b)$.
...
First plot `x` and `y`, as I've entered it in the console, and then create another plot with `y` log-transformed (or log-transform the y-axis using the `log = "y"` argument in `plot()`).
...
If you fit a regression to the now linear relationship, the estimated intercept and slope will be $\text{log}(a)$ and $\text{log}(b)$, respectively, in the equation above.
...
\begin{equation}
\begin{aligned}
  \text{log}(Y) &= \text{log}(aX^b)
  \\
  \text{log}(Y) &= \text{log}(a) + \text{log}(X^b)
  \\
  \text{log}(Y) &= \text{log}(a) + b\text{log}(X)
\end{aligned}
\end{equation}

This equation is now a linear function with a log-transformed DV *and* IV, an intercept equal to $\text{log}(a)$, and a slope equal to $b$.
...
First plot `x` and `y`, as I've entered it in the console, and then create another plot with both `x` and `y` log-transformed (or log-transform both axes using `log = "xy"` in `plot()`).
...
If you fit a regression to the now linear relationship, the estimated intercept and slope will be $\text{log}(a)$ and $b$, respectively, in the equation above.
...
The fourth plot shows that all points fall inside the dashed lines labeled "0.5" (though you might have randomly gotten one or two), so there are no outlier points here, which would bias the estimated coefficients.
...
While it is preferable to not have any model assumptions violated, this is rarely the case when working with real-world data.
...
The true slope is zero; we are using zero because we know that the expected percentage of P-values $\leq$ 0.05 should be 5% (i.e., Type I error rate is 5% when the null hypothesis is true).
...
1. Create two empty arrays, each of which has 1,000 elements (i.e., these are vectors).
...
for(i in 1:length(slope)){
  ...
  # Step 5
  slope[i] <- lm.res$coefficients["x"]
  # Step 6
  p.vals[i] <- summary(lm.res)$coefficients["x", "Pr(>|t|)"]
}
...
I got 0.075, showing that the test is slightly more liberal than your standard regression hypothesis test (i.e., we have slightly more significant P-values than expected, so Type I error rate here is 0.075 instead of the nominal 0.05).
...
1. Create two empty arrays, each of which has 1,000 elements (i.e., these are both vectors).
One array is for the slope, and the other is for the P-value.
You can alternatively create an empty array with 1,000 rows and 2 columns.
2. Initiate the `for()` loop with `for(i in 1:length())`, where `length()` refers to the length of one of the empty arrays.
3. Simulate the IV and DV using the code in the console.
...
This is the expected result with non-independent errors: your test is more likely to falsify the null hypothesis, even if the null is true (i.e., the Type I error rate is too high).
...
All correlation coefficients range from -1 to 1, where -1 is a perfect negative linear relationship between the variables (we'll look at non-linear relationships in *Rank correlation coefficients*), 1 is a perfect positive linear relationship, and 0 is no relationship (where "perfect" is if all points fall on a single straight line).
...
# negative, linear relationship with larger scatter
x <- runif(30)
y <- 2 - 2 * x + rnorm(30, sd = 10)
plot(x, y)
cor(x, y)
```
...
This is done using the same exact code in the linear regression section (click the *Solutions* button above if you want to see the code).
...
We can calculate a P-value or confidence intervals to test if the sample's correlation coefficient is significantly different from zero.
...
You can extract the correlation coefficient with `cor.test(x, y)$estimate`, which is the same as `cor(x, y)`.
Try these out above.
...
Plot the data first and then calculate the Pearson correlation coefficient.
...
You can see that the correlation coefficient is high, but shouldn't it be 1, given that these two variables always increase together according to an exponential function (i.e., this is a **monotonic** relationship)?

Now let's calculate the two rank correlation coefficients, which measure not linearity but monotonicity.
...
Try it on two variables that are less than perfectly correlated (doesn't matter if the relationship is linear, or you can add noise to the exponential relationship in the above console with `y <- exp(x) + rnorm(30)`).
...
Then create a linear regression model, wherein `brainwt` is a function of `bodywt`.
...
plot(lm.res)
# Model assumptions are terribly violated
...
Linear regression is an important statistical tool in its own right, but it also forms the basis for many of the methods in statistics (e.g., t-tests, ANOVA, principal components analysis).
